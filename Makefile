...
## Serve the test HTTP fixtures (the endpoints used by the remote-fetch tests)
.PHONY: server
server:
	$(PHP) -t test/http -S localhost:$(PORT)
...
# No coverage driver is required; use "test-coverage" when a report is wanted.
.PHONY: test
test: ensuretarget
